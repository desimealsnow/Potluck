...
export default function PaymentMethodsScreen({ onBack }: { onBack?: () => void }) {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); const data = await apiClient.listPaymentMethods(); setMethods(Array.isArray(data) ? data : []); }
  ...
  const addPaymentMethod = useCallback(async () => {
    try { Alert.alert('Add Payment Method', 'Setup flow coming soon.'); }
    catch (e: any) { Alert.alert('Add failed', e?.message ?? 'Unknown error'); }
  }, []);
              ...
              methods.map(m => (
                <View key={m.id} style={styles.row}>
                  ...
                  <View style={styles.rowActions}>
                    {!m.is_default && (<Pressable onPress={() => makeDefault(m.id)} style={styles.actionBtn}><Icon name="Star" size={16} color="#7b2ff7" /></Pressable>)}
                    <Pressable onPress={() => remove(m.id)} style={styles.actionBtn}><Icon name="Trash2" size={16} color="#ef4444" /></Pressable>
                  </View>
                </View>
              ))
            ) : (
              <View style={styles.emptyState}>
                <View style={styles.emptyIconContainer}><Icon name="CreditCard" size={48} color="#9CA3AF" /></View>
                <Text style={styles.emptyTitle}>No Payment Methods</Text>
                <Text style={styles.emptySubtitle}>Add a payment method to make purchases and manage your subscription.</Text>
...
const styles = StyleSheet.create({
  ...
  addCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.95)', padding: 16, borderRadius: 12, marginBottom: 16, borderWidth: 2, borderColor: '#7b2ff7', borderStyle: 'dashed' },
  addCardText: { fontSize: 16, fontWeight: '600', color: '#7b2ff7', marginLeft: 12 },
  ...
  actionBtn: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.05)' },
  ...
  emptyActionButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#7b2ff7', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 25 },
