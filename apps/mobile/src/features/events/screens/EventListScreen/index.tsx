...
import { EventListHeader, EventsListView, EventsMapView } from "@/features/events/components";
import Header from "../../components/Header";
import { useDebouncedValue } from "@/hooks/useDebouncedValue";
import { createMaterialTopTabNavigator } from '@react-navigation/material-top-tabs';
import EventDetailsPage from "@/features/events/screens/EventDetailsPage";
import CreateEventScreen from "@/features/events/screens/CreateEvent";
import PlansScreen from "@/features/billing/screens/Plans";
import SettingsScreen from "./SettingsScreen";
import UserProfileScreen from "@/features/account/screens/UserProfile";
import EditProfileScreen from "@/features/account/screens/EditProfile";
import NotificationsScreen from "@/features/account/screens/Notifications";
import UserPreferencesScreen from "@/features/account/screens/UserPreferences";
import SubscriptionScreen from "@/features/billing/screens/Subscription";
import AboutScreen from "@/features/static/screens/About";
import PrivacyScreen from "@/features/static/screens/Privacy";
import HelpScreen from "@/features/static/screens/Help";
import MyItemsScreen from "@/features/items/screens/MyItems";
import PaymentMethodsScreen from "@/features/billing/screens/PaymentMethods";
import InvoicesScreen from "@/features/billing/screens/Invoices";
import MyJoinRequestsScreen from "@/features/events/screens/MyJoinRequests";
...
export default function EventList({ userLocation: propUserLocation }: EventListProps = {}) {
  ...
  const refreshUserLocation = useCallback(async () => {
    try {
      ...
      console.log('refreshUserLocation failed:', error);
    }
  }, []);
  ...
  const reloadWith = useCallback(async (nextTab: EventStatusMobile) => {
    ...
    try {
      ...
      console.warn("Failed to fetch (reloadWith):", error);
      setData([]);
      setHasMore(false);
    } finally {
      setLoading(false);
  ...
  // Removed unused handleRemoveFilter function
              ...
              <View style={{ flex: 1, backgroundColor: '#351657' }} testID="status-filter-container">
                ...
                >
                  ...
                  <MobileTabs.Screen 
                    ...
                    listeners={{
                      focus: () => {
                        setStatusTab("pending-approval");
                        reloadWith("pending-approval");
                      }
                    }}
                    children={() => (
                    <FeatureTabContent
              ...
              <View style={[styles.segmentWrap]} testID="status-filter-container">
                <Segmented
                  options={[
                    ...
                    { key: "pending-approval", label: "Requests" },
                  ]}
                  value={statusTab}
                  onChange={(v) => {
                    ...
                    <View key={req.id} style={[styles.card, { backgroundColor: '#fff' }]}> 
                      ...
                      <View style={{ flexDirection: 'row', marginTop: 10, gap: 8 }}>
                        <Pressable
                          ...
                          onPress={async () => {
                            try { await apiClient.approveJoinRequest(req.event_id, req.id); await reloadWith('pending-approval'); } catch (error) { console.warn('Failed to approve request:', error); }
                          }}
                        >
                        ...
                        <Pressable
                          ...
                          onPress={async () => {
                            try { await apiClient.waitlistJoinRequest(req.event_id, req.id); await reloadWith('pending-approval'); } catch (error) { console.warn('Failed to waitlist request:', error); }
                          }}
                        >
                        ...
                        <Pressable
                          ...
                          onPress={async () => {
                            try { await apiClient.declineJoinRequest(req.event_id, req.id); await reloadWith('pending-approval'); } catch (error) { console.warn('Failed to decline request:', error); }
                          }}
                        >
