            ...
            <View style={styles.card}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                {!sent && phone && phone !== user?.user_metadata?.phone_e164 && (
                  <Pressable style={styles.smallBtn} onPress={async () => { try { await apiClient.post('/user-profile/phone/send', { phone_e164: phone }); setSent(true); Alert.alert('Code Sent', 'We sent an OTP to your phone.'); } catch (e: any) { Alert.alert('Failed', e?.message ?? 'Unknown error'); } }}>
              ...
              {sent && (
                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
                  <TextInput style={[styles.input, { flex: 1, marginRight: 8 }]} placeholder="Enter OTP" keyboardType="number-pad" value={otp} onChangeText={setOtp} />
...
const styles = StyleSheet.create({
  ...
  scrollContainer: { flexGrow: 1, paddingVertical: 16, paddingBottom: 32 },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  loadingText: { color: '#fff', marginTop: 16, fontSize: 16 },
  card: { backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 16, padding: screenWidth < 400 ? 16 : 20, marginBottom: 16 },
  label: { fontSize: screenWidth < 400 ? 14 : 16, fontWeight: '700', color: '#374151', marginBottom: 8, marginTop: 16 },
  subLabel: { fontSize: screenWidth < 400 ? 12 : 14, color: '#6B7280', marginTop: 4, marginBottom: 8 },
  input: { borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 12, paddingHorizontal: screenWidth < 400 ? 12 : 16, paddingVertical: screenWidth < 400 ? 12 : 14, fontSize: screenWidth < 400 ? 14 : 16, color: '#111827', backgroundColor: '#fff' },
  smallBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, backgroundColor: '#A22AD0', marginLeft: 8 },
  smallBtnText: { color: '#fff', fontWeight: '600', fontSize: screenWidth < 400 ? 12 : 14 },
  button: { backgroundColor: '#A22AD0', paddingVertical: screenWidth < 400 ? 14 : 16, borderRadius: 12, alignItems: 'center', marginTop: 24 },
  buttonText: { color: '#fff', fontWeight: '700', fontSize: screenWidth < 400 ? 14 : 16 },
});
