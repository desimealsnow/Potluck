...
export interface PaymentProvider {
  createCheckoutSession(cfg: ProviderConfig, data: CheckoutData): Promise<{ checkoutUrl: string; providerSessionId?: string }>;
  getSubscription(cfg: ProviderConfig, providerSubId: string): Promise<Subscription>;
  cancelSubscription(cfg: ProviderConfig, providerSubId: string, atPeriodEnd?: boolean): Promise<{ success: boolean }>;
