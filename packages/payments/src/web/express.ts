      ...
      for (const ev of events) {
        ...
        switch (ev.name) {
          ...
          case 'invoice.paid':
            ...
            const inv: any = ev.data as any;
            if (!inv.subscriptionId && providerName === 'lemonsqueezy') {
              try {
                const link = (inv.subscriptionsLink as string) || '';
                log.info('Enriching invoice with subscription from LS', { link, orderId: inv.orderId });
                if (link) {
                  const res = await fetch(link, { headers: { 'Authorization': `Bearer ${cfg.credentials.apiKey}`, 'Accept': 'application/vnd.api+json' } as any } as any);
                  ...
                  if (res.ok) {
                    ...
                    if (sub) {
                      ...
                      await container.persistence.upsertSubscription(canonical as any);
                ...
                log.warn('Enrichment fetch failed', { error: (e as Error).message, link: (inv.subscriptionsLink as string) || '' });
              }
            }
            await container.persistence.recordInvoice(inv as any);
            break;
